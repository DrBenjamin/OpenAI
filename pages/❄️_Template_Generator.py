...
with st.form("form"):
    ...
    with st.expander("Verarbeitung"):
        ...
        with options_3.container(border=True):
            ...
            options_3_47 = st.text_input(f"Werden die Daten auf die Eingabe, Veränderung und Löschung protokolliert?", key='options_3_47', value=str(options['OPTION_TEXT'].iloc[54]))
            st.write("Ausstieg")
            ...
            options_3_49 = st.text_input(f"Wie sehen die Ausstiegsszenarien aus?", key='options_3_49', value=str(options['OPTION_TEXT'].iloc[56]))
            ...
            options_3_51 = st.text_input(f"Welche Maßnahmen trifft {cloud}, um Daten vor unbefugtem Zugriff durch Dritte, einschließlich {cloud} selbst, zu schützen?", key='options_3_46', value=str(options['OPTION_TEXT'].iloc[58]))
            options_3_52 = st.text_input(f"Wie unterstützt {cloud} die Verschlüsselung von Daten at-rest, in-transit und in-use?", key='options_3_47', value=str(options['OPTION_TEXT'].iloc[59]))
            options_3_53 = st.text_input(f"Wie unterstützt {cloud} die Verschlüsselung von Daten in-transit?", key='options_3_48', value=str(options['OPTION_TEXT'].iloc[60]))
            options_3_54 = st.text_input(f"Welche Bedeutung hat {cloud}-Verschlüsselungs-Dienst für die Verschlüsselung in-use?", key='options_3_49', value=str(options['OPTION_TEXT'].iloc[61]))
            options_3_55 = st.selectbox(label="Verschlüsselungs-Dienst", options=['AWS Nitro', 'Azure Managed HSM', 'Google Cloud KMS'], key='options_3_50', index=int(options['OPTION_TEXT'].iloc[62]))
            options_3_56 = st.text_input(f"Welche Mechanismen stellt die {cloud}-Lösung zur Verschlüsselung von Patientendaten zur Verfügung? Wie werden Schlüssel gemanaged?", key='options_3_51', value=str(options['OPTION_TEXT'].iloc[63]))
