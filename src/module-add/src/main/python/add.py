...
def add_fn(kunde: str, cloud: str, system: str, on: bool, openai_api_key: str, url: str, port: str) -> str:
    try:
        openai_key = _snowflake.get_generic_secret_string('OPENAI_API')
        print(f"OpenAI API Token: {openai_key}")
    except Exception as e:
        openai_key = f"OpenAI API Token not found ({e})"
        print(openai_key)
    try:
        openai_key_pw = _snowflake.get_username_password('OPENAI_KEY_PW')
        print(f"OpenAI Username & Password: {openai_key_pw}")
    except Exception as e:
        openai_key_pw = f"OpenAI Username & Password not found ({e})"
        print(openai_key_pw)
    ...
    return f"Python version {sys.version}, {openai_key} & {openai_key_pw}" #anzeige_temp
