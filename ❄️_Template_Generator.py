...
#        st.Page(page1, title="Home"),
        st.Page("🤖_OpenAI.py", title="OpenAI ChatBot"),
    ]
}
...
with st.form("form"):
    ...
    with st.expander("Grundsätzliche Informationen"):
    ...
    with st.expander("Auftragsverhältnisse"):
    ...
    with st.expander("Verarbeitung"):
        ...
        with options_3.container(border=True):
            ...
            options_3_46 = st.toggle("Werden besonders schützenswerte Daten verarbeitet?", key='options_3_46', value=eval(options['OPTION_TEXT'].iloc[53]))
            options_3_47 = st.text_input("Werden die Daten auf die Eingabe, Veränderung und Löschung protokolliert?", key='options_3_47', value=str(options['OPTION_TEXT'].iloc[54]))
            st.markdown("---")
            st.markdown("**Ausstieg**")
            options_3_48 = st.text_input(f"Welche Maßnahmen ergreift {cloud} bei Beendigung des Vertragsverhältnisses in Bezug auf die Portabilität?", key='options_3_48', value=str(options['OPTION_TEXT'].iloc[55]))
            options_3_49 = st.text_input("Wie sehen die Ausstiegsszenarien aus?", key='options_3_49', value=str(options['OPTION_TEXT'].iloc[56]))
            options_3_50 = st.text_input(f"Welche Maßnahmen ergreift {cloud} bei Beendigung des Vertragsverhältnisses in Bezug auf die Datenlöschung?", key='options_3_50', value=str(options['OPTION_TEXT'].iloc[57]))
            st.markdown("---")
            st.markdown("**Verschlüsselung**")
            options_3_51 = st.text_input(f"Welche Maßnahmen trifft {cloud}, um Daten vor unbefugtem Zugriff durch Dritte, einschließlich {cloud} selbst, zu schützen?", key='options_3_51', value=str(options['OPTION_TEXT'].iloc[58]))
            options_3_52 = st.text_input(f"Wie unterstützt {cloud} die Verschlüsselung von Daten at-rest, in-transit und in-use?", key='options_3_52', value=str(options['OPTION_TEXT'].iloc[59]))
            options_3_53 = st.text_input(f"Wie unterstützt {cloud} die Verschlüsselung von Daten in-transit?", key='options_3_53', value=str(options['OPTION_TEXT'].iloc[60]))
            options_3_54 = st.text_input(f"Welche Bedeutung hat {cloud}-Verschlüsselungs-Dienst für die Verschlüsselung in-use?", key='options_3_54', value=str(options['OPTION_TEXT'].iloc[61]))
            options_3_55 = st.selectbox(label="Verschlüsselungs-Dienst", options=['AWS Nitro', 'Azure Managed HSM', 'Google Cloud KMS'], key='options_3_55', index=int(options['OPTION_TEXT'].iloc[62]))
            options_3_56 = st.text_input(f"Welche Mechanismen stellt die {cloud}-Lösung zur Verschlüsselung von Patientendaten zur Verfügung? Wie werden Schlüssel gemanaged?", key='options_3_56', value=str(options['OPTION_TEXT'].iloc[63]))
